...
//				||getItemViewType(position)==TYPE_SEND_IMAGE//图片单独处理
        ...
        switch (item.getMsgType()) {
            ...
            case WeTalkConfig.TYPE_LOCATION://位置信息
                try {
                ...
                try {
                    if (text != null && !text.equals("")) {
                        ...
                        if (item.getBelongId().equals(currentObjectId)) {//发送的消息
                            ...
                            /*boolean isExists = WeTalkDownloadManager.checkTargetPathExist(currentObjectId,item);
                            if(!isExists){//若指定格式的录音文件不存在，则需要下载，因为其文件比较小，故放在此下载
                                String netUrl = content.split("&")[0];
                                final String length = content.split("&")[1];
                                *//*WeTalkDownloadManager downloadTask = new WeTalkDownloadManager(mContext,item,new DownloadListener() {
